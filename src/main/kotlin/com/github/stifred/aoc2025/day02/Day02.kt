...
fun main() {
  val ranges = parseInput(day = 2) { it.asIdRanges() }

  solve(part = 1) { ranges.invalidIdSum1() }
  solve(part = 2) { ranges.invalidIdSum2() }
}
...
data class Id(val text: String) {
  fun toLong() = text.toLong()

  fun isInvalid1() = text.length % 2 == 0 && isInvalidOn(text.length / 2)
  fun isInvalid2() = (1..(text.length / 2)).any { isInvalidOn(it) }

  private fun isInvalidOn(check: Int) =
    text.length % check == 0 && text.chunked(check).windowed(2).all { (a, b) -> a == b }
}

class IdRange(from: Long, to: Long) {
  private val ids = (from..to).map { Id("$it") }
